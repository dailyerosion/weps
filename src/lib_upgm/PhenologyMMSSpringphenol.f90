module PhenologyMMSSpringphenol_mod
  ...
  contains
    ...
    subroutine pmms_springphenol(self, plnt, env)
      ...
      call plnt%pars%get("GN_trans_gdd", GN_trans_gdd, succ)
      if( .not. check_return( "GN_trans_gdd", succ ) ) return
      call plnt%pars%get("GN_stress", GN_stress, succ)
      if( .not. check_return( "GN_stress", succ ) ) return
      call plnt%pars%get("GS_trans_gdd", GS_trans_gdd, succ)
      if( .not. check_return( "GS_trans_gdd", succ ) ) return
      call plnt%pars%get("GS_stress", GS_stress, succ)
      if( .not. check_return( "GS_stress", succ ) ) return
      call plnt%pars%get("tverndel", tverndel, succ)
      if( .not. check_return( "tverndel", succ ) ) return
      call plnt%pars%get("height_inc", height_inc, succ)
