...
!$HeadURL$
      ...
      recursive subroutine hybrd(fcn,n,x,fvec,ni,iv,nr,rv,xtol,maxfev,  &
     &                 ml,mu,                                           &
     &                 epsfcn,diag,mode,factor,nprint,info,nfev,fjac,   &
     &                 ldfjac,r,lr,qtf,wa1,wa2,wa3,wa4)
      integer n,ni,nr,maxfev,ml,mu,mode,nprint,info,nfev,ldfjac,lr
      integer iv(ni)
      double precision xtol,epsfcn,factor
      double precision x(n),fvec(n),rv(nr),diag(n),fjac(ldfjac,n),r(lr),&
...
!     where
!
!       fcn is the name of the supplied subroutine which
!         calculates the functions. fcn must be declared
!         in an external statement in the user calling
!         program, and should be written as follows.
!
!         subroutine fcn(n,x,fvec,ni,iv,nr,rv,iflag)
...
!         end
!
!         arrays iv(ni), and rv(nr) are provided to allow the 
!         function to receive integer and real valued parameters without
!         using common blocks.
!
...
!
      iflag = 1
      call fcn(n,x,fvec,ni,iv,nr,rv,iflag)
      nfev = 1
...
!
         iflag = 2
         call fdjac1(fcn,n,x,fvec,ni,iv,nr,rv,fjac,ldfjac,iflag,ml,mu,  &
     &               epsfcn,wa1,wa2)
         nfev = nfev + msum
         if (iflag .lt. 0) go to 300
!
...
!
            iflag = 1
            call fcn(n,wa2,wa4,ni,iv,nr,rv,iflag)
            nfev = nfev + 1
            if (iflag .lt. 0) go to 300
            fnorm1 = enorm(n,wa4)
!
...
!
      ...
      iflag = 0
      if (nprint .gt. 0) call fcn(n,x,fvec,ni,iv,nr,rv,iflag)
      return
!
