...
! Functions for modified signed log-likelihood ratio statistic from:
...
subroutine rstar_z0(n,x,fvec,ni,iv,nr,rv,iflag)
    ...
    fvec(1) = rstar( x(1), rv(1), rv(2), rv(3), rv(4), iv(1) ) - rv(5)

end subroutine

double precision function rstar (psi, psihat, sigsqhat, wbar1, wbar2, nval)
    double precision, intent(in) :: psi, psihat, sigsqhat, wbar1, wbar2
    integer, intent(in) :: nval
    double precision :: sigsqhatpsi, r_psi, u_psi 
    ...
    u_psi = sqrt(nval*1.0)*(psihat-psi)*(sqrt(sigsqhat)/sqrt(sigsqhatpsi)**3)*(1.0/sqrt(0.5+1/sigsqhatpsi))
    !write(*,*) "psihat, psi, sign(psihat-psi) ", psihat, psi, sign(1,psihat-psi)
    !write(*,*) "sigsqhatpsi, r_psi, u_psi ", sigsqhatpsi, r_psi, u_psi

    rstar = r_psi + log(u_psi/r_psi) / r_psi
end function

end module rstar_z0_mod
