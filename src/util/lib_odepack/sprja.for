...
!$HeadURL$
!
      subroutine sprja (isr, neq, y, yh, nyh, ewt, ftem, savf, wm, iwm, &
     &   f, jac)

      use hydro_darcy_mod, only: sls1, slsa
...
! MITER, N, NFE, and NJE.
!-----------------------------------------------------------------------
      sls1(isr)%nje = sls1(isr)%nje + 1
      sls1(isr)%ierpj = 0
      sls1(isr)%jcur = 1
      hl0 = sls1(isr)%h*sls1(isr)%el0
      go to (100, 200, 300, 400, 500), sls1(isr)%miter
! if miter = 1, call jac and multiply by scalar. -----------------------
 100  lenp = sls1(isr)%n*sls1(isr)%n
      do 110 i = 1,lenp
 110    wm(i+2) = 0.0e0
      call jac (neq, sls1(isr)%tn, y, 0, 0, wm(3), sls1(isr)%n)
      con = -hl0
      do 120 i = 1,lenp
 120    wm(i+2) = wm(i+2)*con
      go to 240
! if miter = 2, make n calls to f to approximate j. --------------------
 200  fac = smnorm (sls1(isr)%n, savf, ewt)
      r0 = 1000.0e0*abs(sls1(isr)%h)*sls1(isr)%uround*sls1(isr)%n*fac
      if (r0 .eq. 0.0e0) r0 = 1.0e0
      srur = wm(1)
      j1 = 2
      do 230 j = 1,sls1(isr)%n
        yj = y(j)
        r = max(srur*abs(yj),r0/ewt(j))
        y(j) = y(j) + r
        fac = -hl0/r
        call f (isr, neq, sls1(isr)%tn, y, ftem)
        do 220 i = 1,sls1(isr)%n
 220      wm(i+j1) = (ftem(i) - savf(i))*fac
        y(j) = yj
        j1 = j1 + sls1(isr)%n
 230    continue
      sls1(isr)%nfe = sls1(isr)%nfe + sls1(isr)%n
 240  continue
! compute norm of jacobian. --------------------------------------------
      slsa(isr)%pdnorm = sfnorm (sls1(isr)%n, wm(3), ewt)/abs(hl0)
! add identity matrix. -------------------------------------------------
      j = 3
      np1 = sls1(isr)%n + 1
      do 250 i = 1,sls1(isr)%n
        wm(j) = wm(j) + 1.0e0
 250    j = j + np1
! do lu decomposition on p. --------------------------------------------
      call sgefa (wm(3), sls1(isr)%n, sls1(isr)%n, iwm(21), ier)
      if (ier .ne. 0) sls1(isr)%ierpj = 1
      return
...
! if miter = 4, call jac and multiply by scalar. -----------------------
 400  ml = iwm(1)
      ...
      mband = ml + mu + 1
      meband = mband + ml
      lenp = meband*sls1(isr)%n
      do 410 i = 1,lenp
 410    wm(i+2) = 0.0e0
      call jac (neq, sls1(isr)%tn, y, ml, mu, wm(ml3), meband)
      con = -hl0
      do 420 i = 1,lenp
...
! if miter = 5, make mband calls to f to approximate j. ----------------
 500  ml = iwm(1)
      mu = iwm(2)
      mband = ml + mu + 1
      mba = min(mband,sls1(isr)%n)
      meband = mband + ml
      meb1 = meband - 1
      srur = wm(1)
      fac = smnorm (sls1(isr)%n, savf, ewt)
      r0 = 1000.0e0*abs(sls1(isr)%h)*sls1(isr)%uround*sls1(isr)%n*fac
      if (r0 .eq. 0.0e0) r0 = 1.0e0
      do 560 j = 1,mba
        do 530 i = j,sls1(isr)%n,mband
          yi = y(i)
          r = max(srur*abs(yi),r0/ewt(i))
 530      y(i) = y(i) + r
        call f (isr, neq, sls1(isr)%tn, y, ftem)
        do 550 jj = j,sls1(isr)%n,mband
          y(jj) = yh(jj,1)
          yjj = y(jj)
          r = max(srur*abs(yjj),r0/ewt(jj))
          fac = -hl0/r
          i1 = max(jj-mu,1)
          i2 = min(jj+ml,sls1(isr)%n)
          ii = jj*meb1 - ml + 2
          do 540 i = i1,i2
 540        wm(ii+i) = (ftem(i) - savf(i))*fac
 550      continue
 560    continue
      sls1(isr)%nfe = sls1(isr)%nfe + mba
 570  continue
! compute norm of jacobian. --------------------------------------------
      slsa(isr)%pdnorm = sbnorm (sls1(isr)%n, wm(ml+3), meband, ml, mu, &
     &                           ewt)/abs(hl0)
! add identity matrix. -------------------------------------------------
      ii = mband + 2
      do 580 i = 1,sls1(isr)%n
        wm(ii) = wm(ii) + 1.0e0
 580    ii = ii + meband
! do lu decomposition of p. --------------------------------------------
      call sgbfa (wm(3), meband, sls1(isr)%n, ml, mu, iwm(21), ier)
      if (ier .ne. 0) sls1(isr)%ierpj = 1
      return
