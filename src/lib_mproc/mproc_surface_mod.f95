...
module mproc_surface_mod
  ...
  contains
    ...
    pure subroutine crust (crustf_rm,tillf,crustf,lmosf, lmosm)
...
!     + + + ARGUMENT DECLARATIONS + + +
      real, intent(in) :: tillf     ! Fraction of the surface tilled (0 <= tillf <= 1)
      real, intent(inout) :: crustf ! Current fraction of surface crusted (before & after operation)
      real, intent(in) :: crustf_rm ! Fraction of crust removed (0 <= crustf_rm <= 1)
      real, intent(out) :: lmosf    ! fraction of crusted surface containing loose erodible material
      real, intent(out) :: lmosm    ! mass of loose erodible material on crusted portion of surface
...
!     This was done according to L. Hagen
    ...
    pure subroutine rough ( roughflg, rrimpl, till_i, tillf, &
      ...
      ! + + + ARGUMENT DECLARATIONS + + +
      integer, intent(in) :: roughflg ! 
      real, intent(in) :: tillf       ! Fraction of the surface tilled (0-1)
      real, intent(in) :: rrimpl      ! Assigned nominal RR value for the tillage operation (mm)
      real, intent(inout) :: rr       ! current surface random roughness (mm)
      real, intent(in) :: till_i      ! Tillage intensity factor (0-1)
      integer, intent(in) :: tillay   ! number of layers affected by tillage
      real, intent(in) :: clayf(*)    ! clay fraction of soil
      real, intent(in) :: siltf(*)    ! silt fraction of soil
      real, intent(in) :: rootmass(:) ! mass of roots by layer, pools (kg/m^2)
      real, intent(in) :: resmass(:)  ! mass of buried crop residue by layer, pools (kg/m^2)
      real, intent(in) :: ldepth(*)   ! depth from soil surface of lower layer boundaries
      ...
      ! + + + LOCAL VARIABLES + + +
      integer :: laycnt ! counter for layers
      integer :: laymax ! maximum layer index
      real :: rradj     ! adjusted implement random roughness
      real :: soiladj   ! soil texture adjustment multiplier
      real :: biomass   ! total biomass in the tillage zone
      ...
      if( (roughflg.eq.1).or.(roughflg.eq.3)) then
          ...
          if( rrimpl.gt.rrmin ) then
              ...
              if(rradj.gt.rrmin) then
                  ...
                  rradj = rrmin + (rradj-rrmin) * (0.8*(1-exp(-339.92*biomass))+0.2)
              endif
          endif
      endif
    ...
    subroutine orient( rh, rw, rs, rd, dh, ds, &
      ...
      real, intent(inout) :: rh      ! current ridge height (mm)
      real, intent(out) :: rw      ! current ridge width (mm)
      real, intent(out) :: rs      ! current ridge spacing (mm)
      real, intent(out) :: rd      ! current ridge direction (clockwise from true north)
      real, intent(out) :: dh      ! current dike height (mm)
      real, intent(out) :: ds      ! current dike spacing (mm)
      real, intent(in) :: impl_rh ! implement ridge height (mm)
      real, intent(in) :: impl_rw ! implement ridge width (mm)
      real, intent(in) :: impl_rs ! implement ridge spacing (mm)
      real, intent(in) :: impl_rd ! implement ridge direction (clockwise from true north)
      real, intent(in) :: impl_dh ! implement dike height (mm)
      real, intent(in) :: impl_ds ! implement dike spacing (mm)
      real, intent(in) :: tilld   ! implement tillage depth (mm)
      integer, intent(in) :: rflag ! flag (0-2)
                                   ! 0 - if no ridge, create, else do not modify
                                   ! 1 - set ridge height to input values
                                   ! 2 - adjust ridge height based on tillage depth
