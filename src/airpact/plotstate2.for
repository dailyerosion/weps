! The purpose of this subroutine is to save the variable statement into a file
      ...
      subroutine plotstate2(a,b,sr,day,month,year)
...
! To join the region and subregion index into file name
  !      temp = temp//'a.out'
   ...
   28 format (I5)
      ...
      call fopenk(46,rootp(1:len_trim(rootp))//temp,'unknown') 
...
! soil aggregate properties
 2051 format ('# soil aggregate properties #',/,                         &
     ...
     &     '|',' ahfice ','|','ahzsnd','|','ahzwid ',                    &
     &     '|',' ahzeasurf ','|','ahzsno','|','ahtsno ',                 &
     &     '|',' ahfsnfrz ','|','presswc','|','pressnow ')
...
! print header of daily erosion   
        write (46, 2049)           
        write(46,2079) total,total-suspen, suspen,pmten
...
! Those variables should be removed as there is no effect on erosion result 
        write (46, 2057)
        write (46,*) '  ' 
!        write (46, 2087) ne_sfd84(sr), ne_asvroc(sr),                   &
!     &    ne_wzzo(sr), ne_sfcv(sr)
! crop operation / not used here 
  
        write(46,2058) 
        write (46, 2090, ADVANCE="NO") operat
        write (46, 2091, ADVANCE="YES") crname
! write erosion threshold variables
        write (46,2059)
        write(46,2092)                                                   &
     & 	svroc(1,sr), abrsai(sr),adrsaitot(sr),admftot(sr),abmf(sr),      &                 
     &  admsttot(sr),acanag(sr),acancr(sr)
!    ,sfcr(1,1),sflos(1,1), szrgh(1,sr)              
...
! # The following variables don't affect erosion variables 
      do  i = 1, mnbpls
      write(46,3093)   adzht(i,sr),ad0sla(i,sr),ad0ck(i,sr),             &
...
! crop variables
       write(46,2061) 
      write(46, 2094) acmstandstem(sr),acmstandleaf(sr),                 &
     ...
     &           acrcd(sr),acdkrate(1,sr) 
      ...
      write(46,2062)
      write(46,2095) acddsthrsh(sr), atmstandstem(sr),atmstandleaf(sr),  &
     ...
     &           mdia(msieve+1), nsieve, logcas    
      ...
      write(46,2066) 
!      write(46, 2099)    dmlos(1,1),sf84mn(1,1),smaglos(1,1) 
        do i=1,ntstep-1
         hidx = int(i*23.75/ntstep)+1
         write(46,3001,ADVANCE="NO") ahrwc0(hidx,sr)
        end do
         hidx = 24
         write(46,3001,ADVANCE="YES") ahrwc0(hidx,sr)
...
! save crop variables 
 ...
 !      write(46,3004) amxbr(1,1,sr),amxbr(2,1,sr),amxbr(1,2,sr),          &
 !    &    amxbr(2,2,sr),amzbr(sr),ampbr(sr),amxbrw(sr)
           
!       write(46, 3002) sf84(1,1),sf84mn(1,1)
!     asdagd(1,sr),asfcr(sr), asvroc(1,sr),    &
!     &   asflos(sr),abffcv(sr),dmlos(1,1),sf84mn(1,1),smaglos(1,1),       &
!     &   ahrwcw(1,sr),smaglosmx(1,1)
!      write(46,3003) aslagm(1,sr),as0ags(1,sr),aslagn(1,sr),aslagx(1,sr)  
!     write(46,3003)  sf84ic, sf10ic,sf84mn(1,1), sf84(1,1)       
...
!     &          'ne_sfd84|ne_asvroc|ne_wzzo|ne_sfcv|acanag|acancr') 
 2060   format ('# decomposion variables',/,                            &
     &         '#  ad0sla|ad0ck|admst|addstm|adxstm|adxstmrep','|',     &
     &         'ddsthrsh|admstandstem|admstandleaf|admstandstore','|',  &
     &         'admflatstem|admflatleaf|admflatstore|admflatrootstore', &
     &         '|admflatrootfiber|adgrainf|adm|admst|admf|admbg|admrt|',&
     &         'adrsai|adrlai|adffcv|adfscv|adftcv|admbgz|admrtz','|',  &
     &         'admbgstemz|admbgleafz|admbgstorez|admbgrootstorez','|', &
     &         'admbgrootfiberz|cumddf|covfact|adrsaz|adrlaz','|',      &
     &         'adresevapa|adresevapb|adhyfg|adrbc')
 2061   format ('# Crop variables',/,                                   &
     ...
     &           'abffcv,abfscv,acfcancov,acrcd,acdkrate,acrbc,'        &
     &           'cprevrotation')
