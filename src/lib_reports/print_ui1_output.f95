       ...
       DO p = 1, nperiods
          ...
          IF (period_dates(p)%sy == y) THEN
             ...
             write (UNIT=luogui1,FMT="(1(f10.4,'|'))",ADVANCE="NO")     & !Ave cnt
                      period_report(N_eros_events,p)%val
             write (UNIT=luogui1,FMT="(1(f12.4,'|'))",ADVANCE="NO")     & !Total cnt
                      period_report(N_eros_events,p)%val *              &
                      period_report(N_eros_events,p)%cnt

             IF (period_report(N_eros_events,p)%val > 0.0) THEN
             write (UNIT=luogui1,FMT="(1(f14.4,'|'))",ADVANCE="NO")     & !Gross loss per erosion event
                      period_report(Eros_loss,p)%cnt *                  &
              ...
              !print *, "p/hm/y: ", p, hm,y, period_dates(p), hmonth_dates(hm,y)
                IF ((hmonth_dates(hm,y)%ed == period_dates(p)%ed) .and.  &
                   ...
                   write (UNIT=luogui1,FMT="(3(f10.4,'|'))",ADVANCE="NO") &
                         hmonth_report(Precipi,x,y)%val,                  &
                         hmonth_report(Wind_energy,x,y)%val,              &
                         hmonth_report(Snow_cover,x,y)%val
!                         hmonth_report(Dryness_ratio,x,y)%val
                   GOTO 20
                END IF
             END DO
20           IF (.not. match) THEN  ! No climate info on this date
             ...
             write (UNIT=luogui1,FMT="(6(f10.4,'|'))",ADVANCE="NO")    &
                      ...
                      period_report(Crop_root_mass,p)%val,              &
                      period_report(Crop_stand_height,p)%val,           &
                      period_report(Crop_number_stems,p)%val
             write (UNIT=luogui1,FMT="(8(f10.4,'|'))",ADVANCE="NO")    &
                      ...
                      period_report(Res_buried_mass,p)%val,             &
                      period_report(Res_root_mass,p)%val,               &
                      period_report(Res_stand_height,p)%val,            &
                      period_report(Res_number_stems,p)%val
             write (UNIT=luogui1,FMT="(5(f10.4,'|'))",ADVANCE="NO")    &
                      ...
                      period_report(All_buried_mass,p)%val
             ...
             write (UNIT=luogui1,FMT="(4(f10.4,'|'))",ADVANCE="NO")     &
                      period_report(Surface_Ag_84,p)%val,               &
                      period_report(Surface_Ag_AS,p)%val,               &
                      period_report(Surface_Ag_DN,p)%val,               &
                      period_report(Surface_Ag_CA,p)%val

             write (UNIT=luogui1,FMT="(7(f10.4,'|'))",ADVANCE="YES")    &
                      period_report(Surface_Cr,p)%val,                  &
                      period_report(Surface_Cr_AS,p)%val,               &
                      period_report(Surface_Cr_LM,p)%val,               &
                      period_report(Surface_Cr_TH,p)%val,               &
                      period_report(Surface_Cr_DN,p)%val,               &
                      period_report(Surface_Cr_LF,p)%val,               &
                      period_report(Surface_Cr_CA,p)%val
       ...
       write (UNIT=luogui1,FMT="(1(f10.4,'|'))",ADVANCE="NO")     & !Ave cnt
                 yrly_report(N_eros_events,y)%val
       write (UNIT=luogui1,FMT="(1(f12.4,'|'))",ADVANCE="NO")     & !Total cnt
                 yrly_report(N_eros_events,y)%val *              &
                 yrly_report(N_eros_events,y)%cnt

       IF (yrly_report(N_eros_events,y)%val > 0.0) THEN
       write (UNIT=luogui1,FMT="(1(f14.4,'|'))",ADVANCE="NO")     & !Gross loss per erosion event
                 yrly_report(Eros_loss,y)%cnt *                  &
...
!                         yrly_report(Dryness_ratio,y)%val
    ...
    DO m = 1, 12
       ...
       write (UNIT=luogui1,FMT="(1(f10.4,'|'))",ADVANCE="NO")     & !Ave cnt
                 monthly_report(N_eros_events,m,y)%val
       write (UNIT=luogui1,FMT="(1(f12.4,'|'))",ADVANCE="NO")     & !Total cnt
                 monthly_report(N_eros_events,m,y)%val *              &
                 monthly_report(N_eros_events,m,y)%cnt
 
       IF (monthly_report(N_eros_events,m,y)%val > 0.0) THEN  
       write (UNIT=luogui1,FMT="(1(f14.4,'|'))",ADVANCE="NO")     & !Gross loss per erosion event
                 monthly_report(Eros_loss,m,y)%cnt *                  &
...
!                         monthly_report(Dryness_ratio,m,y)%val
    ...
    DO y = 1, nrot_years*ncycles
       ...
       write (UNIT=luogui1,FMT="(1(f10.4,'|'))",ADVANCE="NO")     & !Ave cnt
                 yr_report(N_eros_events,y)%val
       write (UNIT=luogui1,FMT="(1(f12.4,'|'))",ADVANCE="NO")     & !Total cnt
                 yr_report(N_eros_events,y)%val *              &
                 yr_report(N_eros_events,y)%cnt

       IF (yr_report(N_eros_events,y)%val > 0.0) THEN
       write (UNIT=luogui1,FMT="(1(f14.4,'|'))",ADVANCE="NO")     & !Gross loss per erosion event
                 yr_report(Eros_loss,y)%cnt *                  &
