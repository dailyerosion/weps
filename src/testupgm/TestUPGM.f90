    ...
    !
    !  FUNCTIONS:
    !  TestUPGM - Entry point of console application.
    !
    ...
    gdd_curve = [0.45, 0.35, 0.25, -0.1]    !"ratio" response
    call theModel%plant%phaseCurrent%ptr%phasePars%put("gdd_curve", gdd_curve, success)
    ...
    call theModel%plant%plantstate%state%put("p_depth", i_setter, success)

    ! add phase
    !call theModel%plant%add_phase("weps_initialize", "Initialize", 1)
    ! create all input names
    l_setter = .true.
    call theModel%plant%plantstate%state%put("crop_init", l_setter, success)
    r_setter = 220.0_dp
    call theModel%plant%plantstate%pars%put("plantpop", r_setter, success)
    r_setter = 335.0_dp
    call theModel%plant%plantstate%pars%put("mshoot", r_setter, success)
    r_setter = 8.0_dp
    call theModel%plant%plantstate%pars%put("dmaxshoot", r_setter, success)
    r_setter = 0.05_dp
    call theModel%plant%plantstate%pars%put("huie", r_setter, success)
    r_setter = 1800.0_dp
    call theModel%plant%plantstate%pars%put("thum", r_setter, success)
    i_setter = 120
    call theModel%plant%plantstate%pars%put("pdate", i_setter, success)
    i_setter = 1
    call theModel%plant%plantstate%pars%put("thudf", i_setter, success)
    r_setter = 0.025_dp
    call theModel%plant%plantstate%pars%put("growdepth", r_setter, success)
    r_setter = 28.571428_dp
    call theModel%plant%plantstate%pars%put("storeinit", r_setter, success)
    i_setter = 60
    call theModel%plant%plantstate%pars%put("dtm", i_setter, success)

    ra_setter = [0.04_dp, 0.10_dp, 0.25_dp, 0.6_dp, 1.0_dp, 1.5_dp, 2.0_dp, 2.5_dp, 3.0_dp, 3.5_dp, 4.0_dp, 5.0_dp]
    call env%state%put("laydepths", ra_setter, success)
    ra_setter = [1.5_dp, 3.0_dp, 5.0_dp, 9.0_dp, 14.0_dp, 18.0_dp, 21.0_dp, 22.0_dp, 17.0_dp, 15.0_dp, 10.0_dp, 4.0_dp]
    call env%state%put("mavtmx", ra_setter, success)
    ra_setter = [-4.5_dp, -1.0_dp, 0.0_dp, 3.0_dp, 9.0_dp, 12.0_dp, 15.0_dp, 16.0_dp, 11.0_dp, 9.0_dp, 3.0_dp, -2.0_dp]
    call env%state%put("mavtmn", ra_setter, success)
    r_setter = 38.0_dp
    call env%state%put("amalat", r_setter, success)

    r_setter = 0.0_dp
    call theModel%plant%plantstate%state%put("mstandstem", r_setter, success)
    call theModel%plant%plantstate%state%put("mstandleaf", r_setter, success)
    call theModel%plant%plantstate%state%put("mstandstore", r_setter, success)
    call theModel%plant%plantstate%state%put("mflatstem", r_setter, success)
    call theModel%plant%plantstate%state%put("mflatleaf", r_setter, success)
    call theModel%plant%plantstate%state%put("mflatstore", r_setter, success)
    call theModel%plant%plantstate%state%put("masshoot", r_setter, success)
    call theModel%plant%plantstate%state%put("mtotshoot", r_setter, success)
    ra_setter = [0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp, 0.0_dp]
    call theModel%plant%plantstate%state%put("mbgstemz", ra_setter, success)
    call theModel%plant%plantstate%state%put("mrootstorez", ra_setter, success)
    call theModel%plant%plantstate%state%put("mrootfiberz", ra_setter, success)
    call theModel%plant%plantstate%state%put("height", r_setter, success)
    call theModel%plant%plantstate%state%put("zshoot", r_setter, success)
    call theModel%plant%plantstate%state%put("dstm", r_setter, success)
    call theModel%plant%plantstate%state%put("zrtd", r_setter, success)
    call theModel%plant%plantstate%state%put("zgrowpt", r_setter, success)
    call theModel%plant%plantstate%state%put("fliveleaf", r_setter, success)
    call theModel%plant%plantstate%state%put("trthucum", r_setter, success)
    ...
    call theModel%plant%plantstate%state%put("thardnx", r_setter, success)
    call theModel%plant%plantstate%state%put("grainf", r_setter, success)
    call theModel%plant%plantstate%state%put("leafareatrend", r_setter, success)
    call theModel%plant%plantstate%state%put("stemmasstrend", r_setter, success)
    call theModel%plant%plantstate%state%put("tchillucum", r_setter, success)
    ...
    do while( associated(theModel%plant%phaseCurrent%ptr) )
      ...
      ! update daily inputs
      select case(theModel%plant%phaseCurrent%ptr%phaseLabel)
      case ("Germination")
        soil_moisture = [0.45, 0.35, 0.30, 0.35, 0.32]
