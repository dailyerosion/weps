...
!     only continue if crop is growing
    ...
    subroutine crop_endseason ( isr, bmrotation, bmperod, bm0cfl, &
                                bnslay, mature_warn_flg, crop, cropprev )
...
!     crop model status
      ...
      use biomaterial, only: biomatter, bio_prevday
...
!     day of year
      ...
      if( crop%database%thum .gt. 0.0 ) then
          hui = cropprev%hucum / crop%database%thum
      else
          hui = 0.0
      end if
      ...
      if( (bm0cfl .ge. 0) ) then ! Always print this one now - LEW
        ...
        if(     julday(crop%database%plant_day, crop%database%plant_month, crop%database%plant_rotyr)          &
     &  .gt.julday(lastoper(isr)%day,lastoper(isr)%mon,lastoper(isr)%yr)&
     &  ) then
            adj_plant_yr = crop%database%plant_rotyr - bmperod
        else
            adj_plant_yr = crop%database%plant_rotyr
        end if

        write(UNIT=luoseason(isr),FMT=2010,advance='NO')                &
     &    crop%database%plant_day, crop%database%plant_month, adj_plant_yr,                       &
     &   lastoper(isr)%day, lastoper(isr)%mon, lastoper(isr)%yr, crop%bname,&
     &    cropprev%standstem, cropprev%standleaf, cropprev%standstore,              &
     &    cropprev%flatstem, cropprev%flatleaf, cropprev%flatstore,                 &
     &    bg_stem_sum, root_store_sum, root_fiber_sum,                  &
     &    cropprev%ht, cropprev%stm, cropprev%rtd, cropprev%grainf,                     &
     &    crop%geometry%xstmrep, cropprev%cancov, cropprev%dayap, cropprev%chillucum,           &
     &    cropprev%hucum, crop%database%thum, hui, crop%growth%dayam, cropprev%dayspring
      end if

      ! for annual crops, ALWAYS write out warning message
      ! if harvested before maturity
      if( (hui < 1.0) .and. (mature_warn_flg .gt. 0)                    &
     &    .and. ( (crop%database%idc.eq.1) .or. (crop%database%idc.eq.2)                      &
     &       .or. (crop%database%idc.eq.4) .or. (crop%database%idc.eq.5) ) ) then
         write(UNIT=6,FMT="(1x,3(a),i0,'/',i0,'/',i0,a,f5.1,a,a)")      &
     &       'Warning: ',                                               &
     &       crop%bname(1:len_trim(crop%bname)),                                &
     &       ' harvested ',                                             &
     &       dd, mm, yy,                                                &
