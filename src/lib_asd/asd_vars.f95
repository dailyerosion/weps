MODULE asd_vars
  ...
  REAL, DIMENSION(:), ALLOCATABLE :: xprime_g! Array holding the geometric mean diameters of the sieve cuts (mm)
  REAL, DIMENSION(:), ALLOCATABLE :: mf      ! Array holding the actual number of sieve "cuts" (nsieve+1)
  REAL, DIMENSION(:), ALLOCATABLE :: mf_cum  ! Array holding the cumulative Probability "less than" values at each sieve "cut" (nsieve+1)
...
contains
  ...
  FUNCTION asd_init_vars(ns, minsize, maxsize, mnot, minf) result (ret_status)
    ...
    IF (ALLOCATED (xprime_g) .neqv. .TRUE.) THEN
      ALLOCATE (xprime_g(nsieves+1), STAT = alloc_status)
      IF (alloc_status /= 0) THEN
        write(0,*) "Error allocating xprime_g(nsieves+1) - alloc_status: ", alloc_status
        ret_status = ret_status + alloc_status
        call EXIT (ret_status)
      END IF
    END IF
    ...
    IF (ALLOCATED (mf_cum) .neqv. .TRUE.) THEN
      ALLOCATE (mf_cum(nsieves+1), STAT = alloc_status)
      IF (alloc_status /= 0) THEN
        write(0,*) "Error allocating mf_cum(nsieves+1) - alloc_status: ", alloc_status
        ret_status = ret_status + alloc_status
        call EXIT (ret_status)
      END IF
    END IF
    ...
    DO i = 1, nsieves+1
      IF (i == 1) THEN
        gmdia(i) = sqrt(minsize*sdia(i))
      ELSE IF (i == nsieves+1) THEN
...
!-----------------------------------------------------------------------------------------------------------------------------------------
  ...
  SUBROUTINE asd2mf (gmdx, gsdx, mnot, minf, mfr, mfr_cum)
    ...
    REAL :: x50 = 0.0, xpave = 0.0, xave = 0.0, sigma = 0.0
    REAL :: a = 0.0, b = 0.0
    REAL :: ggmdx = 0.0, ggsdx = 0.0
    REAL :: lngmdx = 0.0
    REAL :: lngsdx = 0.0

    ! Compute transformed sieve dia. sizes
    DO i = 1, nsieves
      IF ((sdia(i) .gt. mnot) .and. (sdia(i) .lt. minf)) THEN
        trsdia(i) = (sdia(i)-mnot) * (minf-mnot) / (minf-sdia(i))
      ELSE
        write(UNIT=0,FMT="(A,i2,4(f8.3))") 'i, sdia(i), mnot, minf are: ',i, sdia(i),' .le. ',mnot,' or ',sdia(i),' .ge. ',minf
      END IF
    END DO
    IF (btest(asd_flg, 0)) THEN
    ...
    IF (btest(asd_flg, 0)) THEN
     write(UNIT=0,FMT="(A,A,f12.3,A,f12.3)",ADVANCE="YES") 'asd2mf: ', 'lngmdx: ', lngmdx, ' lngsdx: ', lngsdx
     write(0,*)
    END IF
...
!      END IF
    ...
    IF (btest(asd_flg, 0)) THEN
       write(UNIT=0,FMT="(A)",ADVANCE="YES") "asd2mf: cumulative fractions of the distribution: mf_cum(i) - P(<val) values"
       write(UNIT=0,FMT="(30(i8))",ADVANCE="YES") (i, i=1,nsieves+1)

       DO i = 1, nsieves+1
         IF (i == 1) THEN
           mfr_cum(i) = mfr(i)
           write(UNIT=0,FMT="(30(f8.3))",ADVANCE="NO") mfr_cum(i)
         ELSE
           mfr_cum(i) = mfr_cum(i-1) + mfr(i)
           write(UNIT=0,FMT="(30(f8.3))",ADVANCE="NO") mfr_cum(i)
         END IF
       END DO
       write(0,*)
    ...
    x50 = (gmdx*minf + mnot*minf - mnot*mnot) / (minf-mnot+gmdx)
    xpave = gmdx*exp(0.5*log(gsdx)*log(gsdx))
    xave = (xpave*minf + mnot*minf - mnot*mnot) / (minf-mnot+xpave)
    sigma = exp(sqrt(2.0)*sqrt(log(xave/x50)))

    IF (btest(asd_flg, 0)) THEN
     write(UNIT=0,FMT="(A,4(A,f12.3))") 'asd2mf: mfr(nsieves+1)', &
          '  x50: ',  x50, ' sigma: ',sigma, ' xpave: ',xpave, ' xave: ', xave
     write(UNIT=0,FMT="(A,4(A,f12.3))") 'asd2mf: mfr(nsieves+1)', &
          ' gmdx: ', gmdx, '  gsdx: ', gsdx, '  mnot: ', mnot, ' minf: ', minf
     write(UNIT=0,FMT="(30(i8))",ADVANCE="YES") (i, i=1,nsieves+1) 
...
!    END DO
!    DO i = 1, nsieves+1
!        xprime_g(i) = trgmdia(i)
!    END DO

! Computing the transformed dia(i) and taking sqrt(trdia(i-1)*trdia(i))
! while using trgmdia(i) and trgmdia(nsieves+1) for the first and last sieve cuts
! since I cannot transform "mnot" and "minf"
    DO i = 1, nsieves+1
       trgmdia(i) = (gmdia(i)-mnot)*(minf-mnot)/(minf-gmdia(i))
    END DO
    DO i = 1, nsieves+1
      IF (i == 1) THEN
        xprime_g(i) = sqrt(trgmdia(i) * trgmdia(i))
      ELSE IF (i == nsieves+1) THEN
        xprime_g(i) = sqrt(trgmdia(i) * trgmdia(i))
      ELSE
        xprime_g(i) = sqrt(trsdia(i-1) * trsdia(i))
      END IF
    END DO
    ...
    a = 0.0
    DO i = 1, nsieves+1
       a = a + mf(i) * log (xprime_g(i))
!       a = a + (mf(i)-mnot)*(minf-mnot)/(minf-mf(i)) * log(xprime_g(i))
    END DO
    b = 0.0
    DO i = 1, nsieves+1
      b = b + mf(i)*log(xprime_g(i))*log(xprime_g(i))
!      b = b + (mf(i)-mnot)*(minf-mnot)/(minf-mf(i)) * log(xprime_g(i))*log(xprime_g(i))
    END DO
    b = sqrt(b - a*a)
    ...
    x50 = (ggmdx*minf + mnot*minf - mnot*mnot) / (minf-mnot+ggmdx)
    xpave = ggmdx*exp(0.5*log(ggsdx)*log(ggsdx))
    xave = (xpave*minf + mnot*minf - mnot*mnot) / (minf-mnot+xpave)
    sigma = exp(sqrt(2.0)*sqrt(log(xave/x50)))
    ...
    IF (btest(asd_flg, 0)) THEN
     write(UNIT=0,FMT="(A,4(A,f12.3))") 'asd2mf: mfr(nsieves+1)', &
          '  x50: ',  x50, ' sigma: ',sigma, ' xpave: ',xpave, ' xave: ', xave
     write(UNIT=0,FMT="(A,4(A,f12.3))") 'asd2mf: mfr(nsieves+1)', &
          'ggmdx: ',ggmdx, ' ggsdx: ',ggsdx, '  mnot: ', mnot, ' minf: ', minf
     write(UNIT=0,FMT="(30(i8))",ADVANCE="YES") (i, i=1,nsieves+1) 
     write(UNIT=0,FMT="(30(f8.3))",ADVANCE="YES") (mfr(i), i=1, nsieves+1)
     write(0,*)
    END IF
