...
module water_erosion_mod

  contains

    subroutine water_erosion(isr, cd,cm,cy, soil, restot, croptot, wp)
...
!
     ...
     &   thetfc,por,tens,wp_cycle)
      ...
      call xinflo(wp_x,wp_efflen,wp_slplen,wp_a,wp_b,wp%qin,            &
     &   wp%qout,wp%peakro,qostar,                                      &
...
!        
      call param(wp%qin,wp%qout,qostar,qshear,wp%qsout,wp_a,wp_b,       &
     &       wp_avgslp,wp_width,wp_rspace,                              &
     &       ktrato,shrsol,tcend,frcsol,frctrl,rrc,wp_npart,            &
     &       wp_frac,wp_dia,wp_spg,                                     &
     &       wp_fall,wp%runoff,wp%effdrn,wp%effint,wp%effdrr,strldn,    &
     &       tcf1,fidel,eata,                                           &
...
!
      if (wp%qin.gt.0.0 .or. wp%qout.gt.0.0)then
        call route(wp%qin,wp%qout,qostar,strldn,ktrato,ainf,binf,cinf,  &
...
!       leaving the profile.
!        
        call sloss(load,tcend,wp_width,wp_rspace,wp%effdrn,theta,       &
     &      wp_slplen,wp_irdgdx,                                        &
     &      wp%qsout,dslost,wp_dsmon, wp_dsyear, wp_dsavg,              &
     &      avsole,wp%qout,frcflw,wp_npart,enrato)
!
...
!
     ...
     & depmin, pdpmin,ndetach,ndepos)
      ...
      if(wp%qout.le.0.0) wp%qsout = 0.0
 
!     write output to erosion events file 

      call write_event(luowepperod(isr),cd, cm, cy, prcp,wp%runoff*1000,&
