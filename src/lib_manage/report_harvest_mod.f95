...
module report_harvest_mod
  ...
  contains
    ...
    subroutine report_harvest( sr, bmrotation, mass_rem, mass_left, &
                               harv_unit_flg, harv_report_flg, thisPlant )
      ...
      type(plant_pointer), pointer :: thisPlant     ! pointer to plant for reporting
 ...
 1001 format(a)
      ...
      if( init_loop(sr) .or. calib_loop(sr) ) then  !initializing or calibrating cycle
        ...
        write(unit=luoharvest_si(sr),fmt=1000,advance='NO') &
          ...
          harv_report_flg, trim(thisPlant%bname), &
          mass_rem, 'kg/m^2', &
          mass_left, 'kg/m^2', &
          harvest_index, "Harvest Index", &
          mass_rem / ( 1.0-thisPlant%database%ywct/100.0 ), &
          'kg/m^2', &
        ...
        if( harv_unit_flg .eq. 0 ) then
          ...
          write(unit=luoharvest_en(sr),fmt=1000,advance='NO') &
            ...
            harvest_index, "Harvest Index", &
            mass_rem*thisPlant%database%ycon/(1.0-thisPlant%database%ywct/100.0), &
            thisPlant%database%ynmu(1:len_trim(thisPlant%database%ynmu)), &
            thisPlant%database%ywct, 'percent water'
        else
          ! the conversion is from dry mass to wet weight
          ...
          write(unit=luoharvest_en(sr),fmt=1000,advance='NO') &
            ...
            KG_per_M2_to_LBS_per_ACRE/( 1.0-thisPlant%database%ywct/100.0 ), &
            'lb/ac', &
            thisPlant%database%ywct, 'percent water'
        end if
 ...
 1050 format(f12.3,'|',a,'|',f12.3,'|',a,'|')
      ...
      if (init_loop(sr) .or. report_loop(sr)) then  ! not a calibrating cycle
        ...
        if (thisPlant%database%baflg == 0) then       ! crop not flagged for calibration
          RETURN
        end if
        ...
        write(unit=luoharvest_calib(sr),fmt=1015,advance='NO') &
          thisPlant%database%plant_day, thisPlant%database%plant_month, thisPlant%database%plant_rotyr, &
          lastoper(sr)%day, lastoper(sr)%mon, lastoper(sr)%yr, trim(thisPlant%bname)
        ...
        write(unit=luoharvest_calib(sr),fmt=1030,advance='NO') &
          harvest_index, "HI", &
          mass_rem/(1.0-(thisPlant%database%ywct/100.0)), "kg/m^2", &
          thisPlant%database%ywct, '% H2O'
        ...
        write(unit=luoharvest_calib(sr),fmt=1040,advance='NO') &
          thisPlant%database%baflg,thisPlant%database%baf,thisPlant%database%yraf, &
          thisPlant%database%ytgt,trim(thisPlant%database%ynmu)
        ...
        write(unit=luoharvest_calib(sr),fmt=1050,advance='NO') &
          thisPlant%database%ytgt/thisPlant%database%ycon, 'kg/m^2', &
          (thisPlant%database%ytgt/thisPlant%database%ycon) * (1.0-(thisPlant%database%ywct/100.0)), 'kg/m^2'
